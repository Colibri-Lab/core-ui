...
Colibri.UI.PointerControl = class {
     ...
     */
    constructor(component, selectionStartHandler, selectionEndHandler, selectingHandler) {
    ...
    __componentMouseDown(event, args) {
        ...
        this._selectionStartHandler(this._startPoint);
    ...
    _createSelectionEventHandlers(point) {
        ...
        this.__documentMouseMove = this.__documentMouseMove || ((e) => {
            ...
            this._selectingHandler({left: Math.min(this._startPoint.left, this._currentPoint.left), top: Math.min(this._startPoint.top, this._currentPoint.top), width: Math.abs(this._currentPoint.left - this._startPoint.left), height: Math.abs(this._currentPoint.top - this._startPoint.top)});
        });

        this.__documentMouseUp = this.__documentMouseUp || ((e) => {
            const bounds = this._component.container.bounds();
            this._endPoint = {left: e.clientX - bounds.left, top: e.clientY - bounds.top};
            this._selectionEndHandler({left: Math.min(this._startPoint.left, this._endPoint.left), top: Math.min(this._startPoint.top, this._endPoint.top), width: Math.abs(this._endPoint.left - this._startPoint.left), height: Math.abs(this._endPoint.top - this._startPoint.top)});
            this._removeSelectionEventHandlers();
        });
