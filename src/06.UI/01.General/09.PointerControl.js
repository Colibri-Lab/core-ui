...
Colibri.UI.PointerControl = class {
    ...
    _createSelectionEventHandlers(point) {
        ...
        this.__documentMouseMove = this.__documentMouseMove || ((e) => {
            ...
            const rect = {
                left: Math.min(this._startPoint.left, this._currentPoint.left),
                top: Math.min(this._startPoint.top, this._currentPoint.top),
                width: Math.abs(this._currentPoint.left - this._startPoint.left),
                height: Math.abs(this._currentPoint.top - this._startPoint.top),
                delta: {
                    left: this._currentPoint.left - this._startPoint.left,
                    top: this._currentPoint.top - this._startPoint.top
                }
            };
            this._selectingHandler(rect, this._args);
        });
        ...
        this.__documentMouseUp = this.__documentMouseUp || ((e) => {
            ...
            const rect = {
                left: Math.min(this._startPoint.left, this._endPoint.left), 
                top: Math.min(this._startPoint.top, this._endPoint.top), 
                width: Math.abs(this._endPoint.left - this._startPoint.left), 
                height: Math.abs(this._endPoint.top - this._startPoint.top),
                delta: {
                    left: this._endPoint.left - this._startPoint.left,
                    top: this._endPoint.top - this._startPoint.top
                }
            };
            this._selectionEndHandler(rect, this._args);
            this._removeSelectionEventHandlers();
        });
