...
Colibri.UI.Spectrum.Waterfall = class extends Colibri.UI.FlexBox {
    ...
    Draw(dataArrayOrObject, show = true) {
        if(Object.isPlainObject(dataArrayOrObject)) {
            this._history.addObject(dataArrayOrObject);
        } else {            
            if (!dataArrayOrObject || dataArrayOrObject.length === 0) return;
            this._history.add(dataArrayOrObject);
        }
    ...
    _crop(floatArray) {
        ...
        const result = new this._dataType(length); // автоматически заполнен нулями
    ...
    Redraw() {
        ...
        const dataType = this._history.dataType;

        let row = 0;
        for (const i in allData) {
            if (!i.isNumeric()) break;
            const rowObject = allData[i];
            const floatArray = this._crop(rowObject.chunk);
            const date = rowObject.date;
            const duration = Math.floor(Number(rowObject.duration / 1000000n / 1000n));
            ...
            const posArray = new dataType(positions);
            const valArray = new dataType(values);
